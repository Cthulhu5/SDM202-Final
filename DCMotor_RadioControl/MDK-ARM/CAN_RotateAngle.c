...
uint16_t gimbal_initialAngle[4];
uint16_t gimbal_offsetAngle[4];
uint16_t gimbal_targetAngle[4];
uint16_t gimbal_currentAngle[4];
uint16_t loopIndex = 0;
uint16_t initialIndex = 0;


void gimbal_RotateAngle(uint16_t motor5_angle, uint16_t motor6_angle, uint16_t motor7_angle, uint16_t motor8_angle)
{
  while (loopIndex < 51)
  {
		CAN_cmd_gimbal(0,3000,0,0);
		HAL_Delay(1);
		for(uint16_t i=0; i<4; i++)
		{
			HAL_CAN_RxFifo0MsgPendingCallback(&hcan2);
			gimbal_currentAngle[i] = get_gimbal_motor_measure_point(i) -> ecd;
		}
		if(initialIndex == 0)
		{
