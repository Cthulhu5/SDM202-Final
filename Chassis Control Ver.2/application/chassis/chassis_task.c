...
static int32_t chassis_angle_broadcast(int16_t yaw, int16_t rev);
...
int16_t GM6020_active;
uint32_t timer;
int32_t GM6020_Delay;
int32_t target_Delay = 2000;
int16_t GM6020_speed = 25000;
int16_t GM6020_idle;
int16_t last_rc_state;
...
void chassis_task(void const *argument)
{
    ...
    while (1)
    {
        ...
        chassis_gyro_updata(&chassis, chassis_gyro.yaw * RAD_TO_DEG, chassis_gyro.gz * RAD_TO_DEG);
				...
				if (rc_device_get_state(&chassis_rc, RC_S1_UP) == E_OK)
				{
					if(last_rc_state == 3)
					{
						timer = HAL_GetTick();
        ...
        {
            vx = (float)p_rc_info->ch2 / 3300 * MAX_CHASSIS_VX_SPEED;
            vy = -(float)p_rc_info->ch1 / 3300 * MAX_CHASSIS_VY_SPEED;
            wz = -(float)p_rc_info->ch3 / 1320 * MAX_CHASSIS_VW_SPEED;
        ...
        else
        {
            ...
            offline_event_disable(OFFLINE_CONTROL_CMD);
				...
				GM6020_Delay = (int32_t)(HAL_GetTick() - timer);
				
				if(GM6020_active == 1 && GM6020_Delay < target_Delay)
				{
					chassis_angle_broadcast((int16_t)(-GM6020_speed / 2.0f),0);
				}
				
				if(GM6020_active == -1 && GM6020_Delay < target_Delay)
				{
					chassis_angle_broadcast((int16_t)(GM6020_speed / 1.0f),0);
				}
				
				if(GM6020_Delay > target_Delay)
				{
					if(GM6020_active == 1)
					{
						chassis_angle_broadcast(0,0);
						GM6020_active = 0;
					}
					
					if(GM6020_active == -1)
					{
						chassis_angle_broadcast((int16_t)(GM6020_speed - 2000),0);
					}
				}
...
/**
  ...
  * @retval void
  */
int32_t chassis_angle_broadcast(int16_t yaw, int16_t rev)
{
...
//    s_yaw_rate = chassis.mecanum.gyro.yaw_gyro_rate * 1000;
    ...
    can1_std_transmit(0x1FF, data, 8);
    return 0;
